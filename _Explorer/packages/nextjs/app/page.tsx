...
const Sprite = ({ name, className }: { name: string; className?: string }) => {
  const spriteMap: Record<string, React.CSSProperties> = {
    // 🚨 VAULT: Nudged X to 10% and Y to 12% to reveal the top-left corner
...
export default function Home() {
  ...
  const stageY = useTransform(scrollYProgress, [0, 1], ["0vh", "350vh"]);

  // --- STRICT DYNAMIC TEXT TIMELINES ---
  ...
  // --- ASSET ANIMATION TIMELINES ---

  // Vault
  const vaultOpacity = useTransform(scrollYProgress, [0.05, 0.15, 1], [0, 1, 1]);

  // XRP Coin Drops 
  const xrpOpacity = useTransform(scrollYProgress, [0.05, 0.1, 1], [0, 1, 1]);
  const xrpY = useTransform(scrollYProgress, [0.15, 0.35], [0, 150]); 
  
  // 🚨 XRP VANISH: Scroll distance reduced by ~25%. Now finishes shrinking/fading at 0.27
  ...
  // Wallet Icon Fades in to catch sXRP
  const walletOpacity = useTransform(scrollYProgress, [0.3, 0.35, 1], [0, 1, 1]);

  // sXRP Emerges from vault and flies LEFT into Wallet
  ...
  // HXT Enters from far RIGHT and flies into Vault
  ...
  return (
    <div ref={containerRef} className="relative h-[450vh] bg-[#0a0f16] text-base-content overflow-hidden">
      
      <motion.div 
        style={{ y: stageY }}
        className="absolute top-0 left-0 h-screen w-full flex flex-col items-center justify-center"
      >
        {/* BACKGROUND LATTICE */}
        <motion.div 
        ...
        {/* DYNAMIC STORYTELLING TEXT */}
        <div className="absolute top-24 z-50 text-center px-5 w-full flex justify-center">
          <motion.div style={{ opacity: text1Opacity, pointerEvents: "none" }} className="absolute">
          ...
          <motion.div style={{ opacity: text2Opacity, pointerEvents: "none" }} className="absolute">
          ...
          <motion.div style={{ opacity: text3Opacity, pointerEvents: "none" }} className="absolute">
            <h1 className="text-3xl md:text-5xl font-bold text-success mb-2 drop-shadow-[0_0_10px_rgba(74,222,128,0.5)]">Minting sXRP</h1>
            <p className="text-lg opacity-70">Synthesizing the Starknet equivalent payload...</p>
          </motion.div>

          <motion.div style={{ opacity: text4Opacity, pointerEvents: "none" }} className="absolute">
            <h1 className="text-3xl md:text-5xl font-bold text-warning mb-2 drop-shadow-[0_0_10px_rgba(251,146,60,0.5)]">Injecting HXT</h1>
            <p className="text-lg opacity-70">Fueling the liquid pool with native Heretic token architecture.</p>
          </motion.div>

          <motion.div style={{ opacity: text5Opacity, pointerEvents: "none" }} className="absolute">
        ...
        {/* ANIMATION STAGE (Center screen) */}
        <div className="relative w-full max-w-4xl h-[400px] z-10 flex items-center justify-center mt-10">
          
          {/* USER WALLET ICON (Target for sXRP) */}
          <motion.div 
            style={{ opacity: walletOpacity }}
            className="absolute z-20 w-20 h-20 bg-base-300/50 rounded-2xl shadow-[0_0_20px_rgba(74,222,128,0.2)] border border-success/30 flex items-center justify-center backdrop-blur-sm left-[10%]"
          >
            ...
            <span className="absolute -bottom-6 text-[10px] uppercase font-bold text-success opacity-80 whitespace-nowrap">Your Wallet</span>
          </motion.div>
          ...
          {/* XRP COIN (Drops from Top, layers ABOVE the Vault) */}
          <motion.div 
            style={{ 
              opacity: xrpInsideOpacity, 
              scale: xrpScale,           
              y: xrpY,
              x: -18 
            }}
            className="absolute z-30 top-[10px] left-1/2 -translate-x-1/2"
          >
            <Sprite name="xrp" />
          </motion.div>

          {/* GREEN PULSE (Draws Left -> Right) */}
          <motion.div 
            style={{ 
              opacity: greenPulseOpacity,
              clipPath: greenPulseClip
            }}
            className="absolute z-10 top-[230px] left-[150px] animate-pulse" 
          >
            <Sprite name="greenPulse" />
          </motion.div>

          {/* sXRP COIN (Emerges Left, shrinks & fades to wallet) */}
          <motion.div 
            style={{ opacity: sxrpOpacity, x: sxrpX, scale: sxrpScale }}
            className="absolute z-30 top-[220px]"
          >
            <Sprite name="sxrp" className="shadow-[0_0_15px_rgba(74,222,128,0.6)]" />
          </motion.div>

          {/* ORANGE FUEL FLOW (Draws Right -> Left) */}
          <motion.div 
            style={{ 
              opacity: orangeFuelOpacity,
              clipPath: orangeFuelClip
            }}
            className="absolute z-10 top-[230px] right-[150px] animate-pulse" 
          >
            <Sprite name="orangeFuel" />
          </motion.div>

          {/* HXT COIN (Enters Right, shrinks into Vault) */}
          <motion.div 
            style={{ opacity: hxtOpacity, x: hxtX, scale: hxtScale }}
            className="absolute z-30 top-[200px]"
          >
